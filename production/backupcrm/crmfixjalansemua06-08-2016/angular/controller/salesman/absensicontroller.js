'use strict';
myAppModule.controller("AbsensiController", ["$q","$rootScope","$scope", "$location","$http","auth","$window","$filter","$timeout","AbsensiService","LocationService","$cordovaSQLite", 
function ($q,$rootScope,$scope, $location, $http,auth,$window,$filter,$timeout,AbsensiService,LocationService,$cordovaSQLite) 
{   
    ...
    document.addEventListener("deviceready", function () 
    {
        ...
        .then(function(result) 
        {
            ...
            {
                alert("Data Absen Untuh Hari Ini Belum Ada Di Local");
                ...
                .then(function(responseabsensi)
                {
                    ...
                    else
                    {
                        ...
                        .then(function(result) 
                        {
                            alert("Absensi Berhasil Disimpan Di Local!");
                        }, 
                        function(error) 
                        {
                            alert("Absensi Gagal Disimpan Di Local: " + error.message);
                        });  
                    }
                },
                function (error)
    ...
    { 
        ...
        if (yakinabsenkeluar == true) 
        {
            ...
            .then (function (response)
            {
                ...
                function(error) 
                {
                    alert("Update Absen Local Gagal: " + error.message);
                });
