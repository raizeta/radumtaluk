                        ...
                        'buttons' => [
                            'update' =>function($url, $model, $key){
                                    return  Html::a('<button type="button" class="btn btn-primary btn-xs" style="width:50px; margin-left:25px">update </button>',
                                                                ['update','CHILD_ID'=>$model->CHILD_ID],[
                                                                'data-toggle'=>"modal",
                                                                'data-target'=>"#modal-child-update",
                                                                'data-title'=> $model->CHILD_ID,
                                                                ]);
                            },
                            'delete' =>function($url, $model, $key){
                                    return  Html::a('<button type="button" class="btn btn-danger btn-xs" style="width:50px; margin-left:25px">delete </button>',
                                                                ['delete','CHILD_ID'=>$model->CHILD_ID],[
                                                                'data-toggle'=>"modal",
                                                                'data-target'=>"#delete-child",
                                                                'data-title'=> $model->CHILD_ID,
                                                                ]);
                            },
                ],
...
$this->registerJs("
         ...
         $('#modal-create-child').on('show.bs.modal', function (event) {
   ...
   ",$this::POS_READY); 
    Modal::begin([
        'id' => 'modal-create-child',
        'header' => '<h4 class="modal-title">LukisonGroup</h4>',
    ]);
    Modal::end();
    $this->registerJs("
        $.fn.modal.Constructor.prototype.enforceFocus = function() {}; 
         $('#modal-child-update').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget)
            var modal = $(this)
            var title = button.data('title') 
            var href = button.attr('href') 
    ...
    ",$this::POS_READY);  
    $this->registerJs("
        $('#delete-child').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget)
            var modal = $(this)
            var title = button.data('title') 
            var href = button.attr('href') 
