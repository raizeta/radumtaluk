...
$drop = ArrayHelper::map(Corp::find()->where('CORP_STS=1 AND CORP_ID="ALG"')->all(), 'CORP_ID', 'CORP_NM');
		...
		<?php 
			...
			echo $form->field($model, 'KD_TYPE')->widget(DepDrop::classname(), [
				...
				'pluginOptions' => [
					...
					'url'=>Url::to(['/dashboard/alg-product/prodak-corp-type']), /*Parent=0 barang Umum*/
					'initialize'=>true,
				], 		
			]);
			...
			echo $form->field($model, 'KD_KATEGORI')->widget(DepDrop::classname(), [
				...
				'pluginOptions' => [
					...
					'url'=>Url::to(['/dashboard/alg-product/prodak-type-kat']),
					'initialize'=>true,
				], 				
			]);
		?>
